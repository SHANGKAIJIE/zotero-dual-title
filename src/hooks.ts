...
async function onNotify(
  ...
  // modify 事件不在此处处理 refresh：
  // Zotero 原生 ItemTree.notify 已通过 _cacheState + rowProvider.notify + _restoreSelection
  // 完整维护 selection 与行重渲染。dual-title 的翻译注入在 _renderItem 中完成，
  // 原生 invalidateRow 即可触发 injectTranslation。此处 refresh 会与原生 selection 恢复竞态。
  // 详见分析报告。
}
